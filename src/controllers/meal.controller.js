...
const calculateNutrition = (item, grams) => {
  ...
  return {
    ...
    vitaminB1:    n(item.vitaminB1),
    vitaminB2:    n(item.vitaminB2),
    vitaminB3:    n(item.vitaminB3),
    vitaminB6:    n(item.vitaminB6),
    vitaminB12:   n(item.vitaminB12),
    folate:       n(item.folate),
    iron:         n(item.iron),
    calcium:      n(item.calcium),
    zinc:         n(item.zinc),
    magnesium:    n(item.magnesium),
    potassium:    n(item.potassium),
    sodium:       n(item.sodium),
    phosphorus:   n(item.phosphorus),
    saturatedFat: n(item.saturatedFat),
    transFat:     n(item.transFat),
  };
};
...
const editMeal = async (req, res) => {
  try {
    ...
    if (!item) {
      return res.status(404).json({ success: false, message: "Food item no longer exists." });
    }

    
    const newPortionUnit = portionUnit || entry.portionUnit;
    const newPortionSize = portionSize || entry.portionSize;
    const newQuantity    = parseFloat(quantity) || entry.quantity;
